...
extension UIImage {
    /** 生成纯色图片 */
    public static func bcgenerateimage(color: UIColor, imageSize: CGSize, cornerRadius: CGFloat = 0, roundingCorners: UIRectCorner = .allCorners) -> UIImage? {
        
        let imageName = ("\(color)" + "si" + "\(imageSize.width) * \(imageSize.height)" + "co" + "\(cornerRadius)" + "ro" + "\(roundingCorners)").bcmd5
        
        let localImage = BSSFileManager.bcreadImage(fileName: imageName, directory: .caches)
        ...
        DispatchQueue(label: "BSSExtensionUIImageSaveImage").async {
            
            BSSFileManager.bcsaveImage(image: image, fileName: imageName, directory: .caches) { progress in
...
extension UIImage {
    /**
     生成渐变色图片 */
    public static func bcgenerateimage(colorArray: [UIColor], gradientType: BSSEnumGradient, imageSize: CGSize, cornerRadius: CGFloat = 0, roundingCorners: UIRectCorner = .allCorners) -> UIImage? {
        
        let imageName = ("\(colorArray)" + "gr" + "\(gradientType)" + "si" + "\(imageSize.width) * \(imageSize.height)" + "co" + "\(cornerRadius)" + "ro" + "\(roundingCorners)").bcmd5
...
extension UIImage {
    /** 压缩图片到指定大小 */
    public func bccompressoriginalimage(size: CGFloat) -> Data {
        let kb: CGFloat = size
