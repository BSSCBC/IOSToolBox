//
//  BSSFileManager+ImageSave.swift
//  IOSToolBox
//
...
public enum BSSSandboxDirectory {
    case documents
    case library
    case caches
    case temp
...
extension BSSFileManager {
    /**
        保存图片到沙盒路径
     */
    public static func bcsaveImage(image: UIImage, fileName: String, directory: BSSSandboxDirectory, progressHandler: @escaping (Double) -> Void, completion: @escaping (Bool) -> Void) {
        // 获取沙盒目录
...
extension BSSFileManager {
    /**
       从沙盒路径取出图片
     */
    public static func bcreadImage(fileName: String, directory: BSSSandboxDirectory) -> UIImage? {
        // 获取沙盒目录
